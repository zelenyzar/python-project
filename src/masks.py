...
console_handler = logging.FileHandler("logs/masks.log", mode="w", encoding="utf-8")
...
def get_mask_card_number(numbers_card: Union[str]) -> Union[str]:
    ...
    for num in range(len(numbers_card)):
        if 6 <= num <= 11:
            ...
            logger.debug("Скрыта часть номера карты")
        else:
